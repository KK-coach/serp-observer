...
const d4s = require('./src/dataforseo');
const { parseSerpItems } = require('./src/parser');
...
async function postSerpTasks(req, res) {
  try {
    ...
    let totalPosted = 0;
    const chunks = chunkArray(taskPayload, config.maxTasksPerPost);
    ...
    res.status(200).json({ runId, totalPosted });
  } catch (err) {
...
async function fetchSerpResults(req, res) {
  try {
    ...
    res.status(200).json({ processed: tasks.length, completed });
  } catch (err) {
...
async function postSearchVolumeTasks(req, res) {
  try {
    ...
    res.status(200).json({ taskId: taskObj?.id, keywordCount: keywords.length });
  } catch (err) {
