...
const config = {
  ...
  serpDevices: (process.env.SERP_DEVICES || 'desktop,mobile').split(',').map((d) => d.trim().toLowerCase()).filter(Boolean),
  loadAsyncAiOverview: ['true', '1', 'yes'].includes((process.env.LOAD_ASYNC_AI_OVERVIEW || '').trim().toLowerCase()),
  fetchBatchLimit: Number(process.env.FETCH_BATCH_LIMIT || 50),
